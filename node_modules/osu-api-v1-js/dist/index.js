import { Categories, Genres, Languages } from "./beatmap.js";
import { MultiplayerModes, WinConditions } from "./match.js";
import { Mods } from "./mods.js";
import { Gamemodes, getLength, getURL } from "./misc.js";
export { Gamemodes, Mods };
export { Categories, Genres, Languages };
export { MultiplayerModes, WinConditions };
 ...
 */
function correctType(x, force_string) {
...
export class APIError {
     ...
     */
    constructor(message, server, endpoint, parameters) {
...
export class API {
     ...
     */
    constructor(key, verbose = "none", server = "https://osu.ppy.sh/api/") {
        this.key = key;
     ...
     */
    async request(endpoint, parameters, number_try = 1) {
        const max_tries = 5;
        let err = "none";
        let to_retry = false;
     ...
     */
    async getUser(gamemode, user) {
        const lookup = user.user_id !== undefined ? `u=${user.user_id}&type=id` : `u=${user.username}&type=string`;
        const response = await this.request("get_user", `m=${gamemode}&${lookup}`);
        return response[0];
    }
    /**
     ...
     */
    async getUserBestScores(limit, gamemode, user) {
        const lookup = user.user_id !== undefined ? `u=${user.user_id}&type=id` : `u=${user.username}&type=string`;
        const scores = await this.request("get_user_best", `${lookup}&m=${gamemode}&limit=${limit}`);
        scores.forEach((s) => s.enabled_mods = Mods.bitsToArray(s.enabled_mods));
        return scores;
    }
    /**
     * @param limit The maximum number of Scores to get, **cannot exceed 100**
     ...
     * @returns A Promise with an array of `Scores` set by the `User` within the last 24 hours in a specific `Gamemode`
     */
    async getUserRecentScores(limit, gamemode, user) {
        const lookup = user.user_id !== undefined ? `u=${user.user_id}&type=id` : `u=${user.username}&type=string`;
     ...
     */
    async getBeatmap(beatmap, mods = [], gamemode) {
        ...
        const response = await this.request("get_beatmaps", `b=${beatmap.beatmap_id}${details}`);
     ...
     */
    async getBeatmaps(limit, gamemode, beatmap, mods = [], set_owner, since) {
        mods = Mods.removeUnsupported(mods);
        const mods_bits = Mods.arrayToBits(mods);
        ...
        const response = await this.request("get_beatmaps", `limit=${limit}${mode}&${convert}${lookup}`);
        response.forEach((b) => b.max_combo = b.max_combo !== null ? b.max_combo : 0); // turn null max_combo into 0
     ...
     */
    async getBeatmapScores(limit, gamemode, beatmap, user, mods = []) {
     ...
     */
    async getMatch(id) {
        const match = await this.request("get_match", `mp=${id}`);
        match.games.forEach((g) => {
            g.mods = Mods.bitsToArray(g.mods);
            g.scores.forEach((s) => {
                s.enabled_mods = s.enabled_mods === null ? null : Mods.bitsToArray(s.enabled_mods);
            });
        });
        return match;
    }
     ...
     * see https://docs.ripple.moe/docs/api/peppy
     */
    async getReplay(gamemode, replay) {
